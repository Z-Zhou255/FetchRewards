...
class ListRepositoryTest : DescribeSpec({
    ...
    describe("Testing ListRepository") {
        it("should return filtered and sorted items") {
            ...
            coEvery { mockApiService.fetchListRewards() } returns mockItems
            val result = runBlocking { repository.getListRewardItems().first() }
    ...
    it("should emit an empty list when an exception occurs") {
        coEvery { mockApiService.fetchListRewards() } throws RuntimeException("Network error")
        val result = runBlocking { repository.getListRewardItems().first() }
